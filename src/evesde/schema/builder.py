...
def _safe_identifier(name: str) -> str:
    ...
    (см. reference/eve_sde_full_schema.sql) -- добавление префикса/суффикса
    к уже почти предельно длинному имени таблицы может вывести сгенерированное
    имя ограничения за лимит.
...
def build_metadata(
    ...
    ``schema`` -- необязательная схема/база-приёмник (используется загрузчиком,
    `etl/loader.py`, для теневой загрузки на PostgreSQL/MySQL:
    `MetaData(schema=...)` автоматически
